...
int main()
{
    ...
    while(isspace(*(words+(lenght-1)))){
        lenght--; //get \n or other stuff like spacecs out of the end
    }
        //printf("%c", *(words+i));
...
if (*first_position<0){ //kol dar nera rasta zodzio pirmos raides vieta
        ...
        if (*last_position<0){ //kol dar nera rasta zodzio pirmos raides vieta
        if (isspace(*(words+i))|| i==(lenght-1)){ //if theres a space or this is the last word 
            *last_position = i-1;
            if (i==(lenght-1)){ //if the word is the last symbol
            *last_position = i;
        }
        }
        }
